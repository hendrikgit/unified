...
{
    if (Config::Get<int>("CHARLIST_SORT_BY_LAST_PLAYED_DATE", 0) < 1)
        return;

    s_NumCharactersToSort = Config::Get<int>("CHARLIST_SORT_BY_LAST_PLAYED_DATE", 1);
    if (s_NumCharactersToSort > 2097152) // 2^7 * 2^7 * 2^7 = 2^21 = 2097152, see below
    {
        LOG_ERROR("Plugin NOT active! The maximum value for CHARLIST_SORT_BY_LAST_PLAYED_DATE is %d", 2097152);
        return;
    }
